"""Reusable statistical modeling utilities shared across notebooks and scripts.

These helpers avoid project-specific assumptions; pass your feature candidates or
business mappings from the calling project.
"""
...
__all__ = [
    ...
    "maybe_sample_df",
    "build_light_regression_models",
    "build_light_classification_models",
    "choose_best_model",
    "compute_shap_and_top",
    "attach_actions",
    "plot_metrics_bar",
]
...
def compute_shap_and_top(
    ...
    feature_names = (
        ...
        else np.array([f"f{i}" for i in range(X_train_enc.shape[1])])
    )
...
def attach_actions(top_df: pd.DataFrame, driver_actions: Mapping[str, str], fallback: str | None = None) -> pd.DataFrame:
    """Map features to business actions using a provided mapping."""
